...
module Api
  module V1
    class InputCostsController < BaseController
      ...
      def index
        costs = @field.input_costs.order(season_year: :desc, category: :asc)
        render_success(costs.map { |c| input_cost_json(c) })
      end

      def create
        cost = @field.input_costs.build(input_cost_params)
        if cost.save
      ...
      def input_cost_params
        params.require(:input_cost).permit(:season_year, :category, :amount_per_acre, :notes)
      end

      def input_cost_json(cost)
        cost.as_json(only: %i[
          id field_id season_year category amount_per_acre notes created_at updated_at
        ])
      end
    end
  end
